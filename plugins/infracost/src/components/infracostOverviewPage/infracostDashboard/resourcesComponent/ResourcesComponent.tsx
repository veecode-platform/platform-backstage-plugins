...
export const ResourcesComponent: React.FC<ResourcesComponentProps> = props => {
  ...
  const { root, title, chartStyles, tableStyles, expandDetails } =
    useResourcesComponentStyles();
      ...
      tableDispatch(
        addRow({
          ...
          monthly_cost: resource.monthlyCost
            ? `$ ${Number(resource.monthlyCost).toFixed(2)}`
            : 'Depends on usage*',
        }),
      );
    ...
    <>
      <div className={root}>
        <div className={title}>
          <Chip
            variant="outlined"
            label={`Generated to ${dayjs(timeGenerated).fromNow()}`}
          />
        </div>
