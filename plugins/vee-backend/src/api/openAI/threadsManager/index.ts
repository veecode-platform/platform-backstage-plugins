...
export class ThreadsManager extends OpenAIClient implements IThreadsManager {
  ...
  async checkRunStatus({ threadId, runId }: CheckRunStatusParams) {
    try {
      ...
      throw new Error(
        `Erro to check run status [run_id: ${runId}] :  ${error}`,
      );
    }
  }
  ...
  async listMessages(threadId: string) {
    try {
      ...
      throw new Error(
        `Erro to message list in thread [thread_id: ${threadId}] :  ${error}`,
      );
    }
  }
  ...
  async deleteThread(threadId: string) {
    try {
      const response = await this.client.beta.threads.delete(threadId);
      return {
        status: 'ok',
        ...response,
      };
      ...
      throw new Error(
        `Erro to delete thread [thread_id: ${threadId}] :  ${error}`,
      );
    }
  }
}
