...
export class OpenAIApi extends OpenAIClient implements IOpenAIApi {
  ...
  async submitDataToVectorStore({
    repoName,
    files,
  }: SubmitDataToVectorStoreParams) {
    try {
      this.logger.info('Check VectorStore Available...');
      const existingStores = await this.client.vectorStores.list();
      const existingStore = existingStores.data.find(
        (store: VectorStore) => store.name === repoName,
      );
      ...
      if (existingStore && existingStore.status !== 'expired') {
        ...
        this.logger.info(
          `VectorStore not found. Creating a new one for repo: ${repoName}`,
        );
        vectorStoreId = await this.vectorStoreManager.createVector(repoName);
      }
      const response = await this.vectorStoreManager.uploadFiles(
        vectorStoreId,
        files,
      );
      return {
  ...
  async initializeAssistant({
    ...
    try {
      ...
      const existingAssistant = repoName
        ? assistants.data.find((a: any) => a.name === repoName)
        : false;
      const model =
        modelType === AIModel.customModel && config.templateGeneration
          ? config.templateGeneration.model
          : config.model;

      if (!existingAssistant)
        return this.assistantAI.initializeAssistant({
          vectorStoreId,
          repoName,
          modelType,
          model,
  ...
  async startChat({
    ...
    try {
      ...
      const assistant = await this.initializeAssistant({
        vectorStoreId,
        repoName,
        repoStructure,
        modelType,
  ...
  async executeAndCreateRun({
    assistantId,
    threadId,
  }: ExecuteAndCreateRunParams) {
    const run = await this.threadsManager.executeAndCreateRun({
      threadId,
      assistantId,
    });
    return run;
  ...
  async getChat({ assistantId, threadId, message }: GetChatParams) {
    try {
      ...
      let runValidate = await this.threadsManager.checkRunStatus({
        threadId,
        runId: run.id,
      });
      while (runValidate.status !== 'completed') {
        await new Promise(resolve => setTimeout(resolve, 1000));
        runValidate = await this.threadsManager.checkRunStatus({
          threadId,
          runId: run.id,
        });
      }
      ...
      const analysisResponse = latestMessages.data
        ...
        .map((msg: any) => {
          const contentBlock = msg.content?.find(
      ...
      // parse response
      const analysisResponseParsed =
        validateAssistantResponse(analysisResponse);
