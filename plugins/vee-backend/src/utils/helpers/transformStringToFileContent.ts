...
function transformToObjectWithFileContent(obj: Record<string, any>): FileContent[] {
...
export function transformStringToObject(jsonString: string): FileContent[] {
  const newJsonString = jsonString.replace(
    /([{,]\s*)(\.\w+|[a-zA-Z0-9_\-.]+)\s*:/g,
    '$1"$2":'
  );
  const objTransformed: Record<string, any> = JSON.parse(newJsonString); 
  const fileContentsData = transformToObjectWithFileContent(objTransformed);
