...
export class GithubManager implements IGithubManager {
  ...
  private readonly token: string;
    ...
    async getOctokit(hostname: string = 'github.com') {
    ...
    async createPullRequest(
      ...
      // Obter referência do branch base
      ...
      // Iterar sobre os arquivos recebidos e criar/atualizar no repositório
      for (const file of files) {
        const { relativePath, content } = file;
    
        let fileSha: string | undefined;
        try {
          // Verificar se o arquivo já existe no branch
        ...
        // Criar ou atualizar o conteúdo do arquivo no branch
        await octokit.repos.createOrUpdateFileContents({
          owner,
          repo,
          path: relativePath,
          message: `${title}: Update ${relativePath}`,
          content: Base64.encode(content), // Codificar o conteúdo para Base64
          branch: branchName,
          sha: fileSha,
        }).catch((e: any) => {
          throw new Error(
