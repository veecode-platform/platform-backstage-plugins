...
export const TemplateOutput = () => {
  ...
  const [pullRequestResponseState, pullRequestResponseDispatch] =
    React.useReducer(
      PullRequestResponseReducer,
      initialPullRequestResponseState,
    );
  const [codeBlock, setCodeBlock] = React.useState<CodeBlockProps>({
    language: 'javascript',
  ...
  const { catalogLocation } = useConfig(
    entityInfoState?.engine.toLowerCase() ?? 'openai',
  );
  const navigate = useNavigate();
  const { root, codeSection, footer, buttonsGroup } = useTemplateOutputStyles();

  const handleCode = (lang: string, code: string) =>
    setCodeBlock({ language: lang, code });
  ...
  const createPullRequest = async () => {
    if (
      catalogLocation &&
      templateOutputState &&
      templateOutputState.files.length > 0
    ) {
      const response = await saveTemplateToCatalog(
        catalogLocation as string,
        templateOutputState.files,
      );
      if (response) {
  ...
  const templateFiles = React.useMemo(() => {
    if (templateOutputState) {
      const fileSelected = templateOutputState.files.find(
        file => file.name === 'template.yaml',
      );
      if (fileSelected) {
