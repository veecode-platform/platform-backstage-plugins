...
export const IncrementedInput: React.FC<IncrementedInputProps> = props => {
  ...
  const [optionState, optionDispatch] = React.useReducer(
    OptionReducer,
    initialOptionState,
  );
  const [optionsListState, optionsListDispatch] = React.useReducer(
    OptionsListReducer,
    initialOptionsListState,
  );
  ...
  const addOptionToList = () => {
    if (optionState.label !== '' && optionState.prompt !== '') {
      ...
      const newOption = {
        ...optionState,
        id: optionState.id ? optionState.id : generatedId,
      } as IOption;
          ...
          <Grid2 size={{ md: 10 }}>
            <Box className={inputGroup}>
              ...
              <TextField
                ...
                value={
                  optionState.prompt ??
                  'Ensure the response includes modular, maintainable code ready for a pull request...'
                }
                label="Prompt"
                name="prompt"
              />
            ...
            <div className={footerButton}>
              <Button variant="danger" onClick={() => setShowInputs(false)}>
                Close
              </Button>
            </div>
          </Grid2>
