...
export const WorkFlowActions: React.FC<WorkFlowActionsProps> = props => {
  ...
  const filters = workflowsByAnnotation
    ? workflowsByAnnotation.flatMap(w => w.workflow)
    : [];
  ...
  const updateWorkflowState = (workflowIdParam: number) => {
    ...
    const checkWorkflowStatus = async () => {
      ...
      if (allWorkflowsState) {
        const workflowNewState = allWorkflowsState.find(
          w => w.id === workflowIdParam,
        );
        if (workflowNewState) {
  ...
  const handleStartWorkflow = async () => {
    if (
      status === StatusWorkflowEnum.pending ||
    ...
    const response = await handleStartWorkflowRun(
      workFlowSelected?.id as number,
    );
        ...
        switch (statusParams) {
          ...
          case StatusWorkflowEnum.default:
            if (
              parameters &&
              parameters.length > 0 &&
              Object.keys(inputsParamsState).length === 0
            ) {
  ...
  return (
    <>
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.skipped && (
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.canceled && (
          <Tooltip title="Run Workflow" placement="right">
            <ReplayIcon
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.timeOut && (
          <Tooltip title="Run Workflow" placement="right">
            <ReplayIcon
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.aborted && (
          <Tooltip title="Run Workflow" placement="right">
            <ReplayIcon
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.failure && (
          <Tooltip title="Run Workflow" placement="right">
            <ReplayIcon
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.success && (
          <Tooltip title="Run Workflow" placement="right">
            <ReplayIcon
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.completed && (
          <Tooltip title="Run Workflow" placement="right">
            <SyncIcon
              onClick={() => handleClickActions(StatusWorkflowEnum.success)}
            />
      ...
      {status.toLocaleLowerCase() === StatusWorkflowEnum.completed &&
        conclusion?.toLocaleLowerCase() === StatusWorkflowEnum.default && (
          <Tooltip title="Run Workflow" placement="right">
            <ReplayIcon
