...
const WorkflowTable: React.FC<WorkflowTableProps> = props => {
  ...
  const { action, source, clickable, title, options, name } =
    useWorkflowTableStyles();
      ...
      name: (
        <div className={name}>
          ...
          <Tooltip
            title={
              item.lastRunId
                ? 'Open the actions panel on github...'
          ...
          >
            <Link
              to={
                item.lastRunId
                  ? `https://${hostname}/${projectName}/actions/runs/${item.lastRunId}`
                  : '#'
              }
              target="_blank"
            >
